...
pub fn derive_attribute_collector(input: TokenStream) -> TokenStream {
    ...
    let gen = quote! {
        ...
        impl #struct_impl {
            pub fn add<'a: 'b, 'b, D: surrealdb::Connection>(self, conn: &'a surrealdb::Surreal<D>)-> surrealdb::method::Content<'b, D, Option<Vec<surrealdb_extras::RecordData<#struct_impl>>>> {
                conn.create(#struct_name).content(self)
            }

            pub fn insert<'a: 'b, 'b, D: surrealdb::Connection>(self, conn: &'a surrealdb::Surreal<D>, id: surrealdb::RecordIdKey)-> surrealdb::method::Content<'b, D, Option<surrealdb_extras::RecordData<#struct_impl>>> {
