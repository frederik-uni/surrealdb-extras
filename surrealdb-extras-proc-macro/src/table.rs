...
pub fn derive_attribute_collector(input: TokenStream) -> TokenStream {
    ...
    let gen = quote! {
        ...
        impl #struct_impl {
            pub fn add<'a: 'b, 'b, D: surrealdb::Connection>(self, conn: &'a surrealdb::Surreal<D>)-> surrealdb::method::Content<'b, #struct_impl, Option<Vec<surrealdb_extras::RecordData<#struct_impl>>>> {
